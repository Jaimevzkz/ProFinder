...
interface GetIndChatsUseCase {
    suspend operator fun invoke(otherUid: String): Result<Flow<List<ChatMsgModel>>, FirebaseError.Realtime>
}
...
class GetIndChatsUseCaseImpl @Inject constructor(
    ...
    override suspend operator fun invoke(otherUid: String): Result<Flow<List<ChatMsgModel>>, FirebaseError.Realtime> =
        repository.getIndividualChat(getUidDataStoreUseCase(), otherUid)
}